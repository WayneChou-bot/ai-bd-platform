...
export class TavilySearchAdapter implements LeadSourceAdapter {
  ...
  async discover(q: DiscoveryQuery, ctx: { now: () => Date }): Promise<DiscoveryResult[]> {
    ...
    const out: DiscoveryResult[] = [];
    for (const query of this.buildQueries(q.icp)) {
      ...
      for (const r of json.results ?? []) {
        ...
        if (/(linkedin|glassdoor|indeed|wikipedia|crunchbase|reddit|medium|youtube|twitter|x)\.com$/.test(host)) continue;
...
// ---------------------------------------------------------------------------
export async function createSourceAdapters(cfg: AppConfig): Promise<LeadSourceAdapter[]> {
  if (cfg.mode === "demo") {
    const { FixturePoolAdapter } = await import("./fixture-pool");
    return [new FixturePoolAdapter()];
  }
  const adapters: LeadSourceAdapter[] = [];
  if (cfg.searchApiKey) adapters.push(new TavilySearchAdapter(cfg.searchApiKey));
