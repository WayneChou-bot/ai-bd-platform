    ...
    <>
      ...
      {sp.msg && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-engage/30 bg-engage/10 px-3 py-2 text-sm text-engage">
          <span>{sp.msg}</span>
          <Link href={`/leads?project=${project.id}&view=qualified`} className="font-medium underline decoration-dotted hover:decoration-solid">{t("View qualified leads")} →</Link>
        </div>
      )}
