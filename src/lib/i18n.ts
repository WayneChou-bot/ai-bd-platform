...
const zh: Record<string, string> = {
  ...
  // discover
  "Controlled sources only (§12)": "僅使用受控來源（§12）", "demo seed pool": "示範種子池", "Run full pipeline": "執行完整管線",
  ...
  "View qualified leads": "查看合格潛客", "Rejected": "已排除", "added": "已新增",
  ...
  // projects
  "Every BD campaign belongs to a project (§9).": "每個 BD 活動都隸屬於一個專案（§9）。", "New project": "新增專案", "leads": "位潛客",
  ...
  "product": "產品", "understanding": "理解", "activity_tab": "活動", "Product profile": "產品資料", "Save": "儲存", "Pipeline for this project": "此專案的管線",
  "Product Understanding": "產品理解", "pending": "待處理", "Discovery": "探索", "Open Discover": "前往探索",
  "The platform must understand what is being promoted before it looks for anyone to promote it to (§10).": "平台必須先理解要推廣的是什麼，才會去找該推廣給誰（§10）。",
  "Run Product Understanding Agent": "執行產品理解代理", "README": "README", "optional; treated as untrusted data": "選填；視為不可信資料", "Manual notes": "手動備註",
  ...
  "Not run yet. The agent returns structured JSON only (§10) — the UI never shows raw model text.": "尚未執行。代理只回傳結構化 JSON（§10）— 介面不會顯示原始模型文字。",
  ...
  "Human-in-the-loop queue: nothing leaves without approval (§3.2, §19).": "人工審核佇列：未經核准不會寄出任何訊息（§3.2、§19）。",
  ...
  // analytics
  "Every number is recomputed from rows — nothing is stored as a metric (§21, §47).": "每個數字都由資料列即時計算 — 不儲存任何指標（§21、§47）。",
  ...
  "Lead source performance": "來源表現", "Qualified rate": "合格率", "Positive": "正向", "Agent operations": "代理營運", "latency p50 · failure rate · tokens (§47)": "延遲 p50・失敗率・token（§47）",
  "Agent": "代理", "Runs": "執行", "Failure rate": "失敗率", "p50 latency": "p50 延遲", "Tokens": "Token",
  // agents page
  "Every run is a row: explicit state, latency, tokens, retries, errors. No chain-of-thought stored (§42).": "每次執行都是一列：明確狀態、延遲、token、重試、錯誤。不儲存思維鏈（§42）。",
  "Run": "執行", "Latency": "延遲", "Retries": "重試", "Output / Error": "輸出／錯誤",
  "Search leads…": "搜尋潛客…", "Clear": "清除", "No leads match.": "沒有符合的潛客。",
  // settings
  "Runtime status — read-only. Values come from environment variables (§43: keys never reach the browser).": "執行狀態 — 唯讀。值來自環境變數（§43：金鑰不會傳到瀏覽器）。",
  ...
  "entities_tab": "追蹤實體", "Tracked entities": "追蹤實體",
